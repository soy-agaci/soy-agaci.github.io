-- Migration: Add explicit primary_person_id field to partnership_revisions

alter table public.partnership_revisions
  add column if not exists primary_person_id uuid references public.people (id) on delete restrict;

-- Update get_family_graph to return primary_person_id in partnership objects
...
visible_source_submission_ids as (
  select r.submission_id from public.family_membership_revisions r
  join selected_memberships_final m on m.id = r.family_membership_id
  where r.status in ('approved', 'superseded') and r.submission_id is not null
  union select r.submission_id from public.person_revisions r
  join visible_people p on p.person_id = r.person_id
  where r.status in ('approved', 'superseded') and r.privacy = 'public' and r.submission_id is not null
  union select r.submission_id from public.life_event_revisions r
  join selected_life_events e on e.id = r.life_event_id
  where r.status in ('approved', 'superseded') and r.submission_id is not null
  union select r.submission_id from public.partnership_revisions r
  ...
  'partnerships', coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'current_revision', case when current.id is null then null else jsonb_build_object(
        ...
        'primary_person_id', current.primary_person_id,
...
grant select on table public.families to anon, authenticated, service_role;

-- Update import_family_sheet to populate primary_person_id for partnerships
...
begin
  ...
  with input as (
    select value->>'key' as key,
           ...
           value->>'primary_person_legacy_id' as primary_person_legacy_id,
           value->>'date_start' as date_start,
           value->>'date_text' as date_text
  ...
  ids as (
    select key, date_start, date_text,
           ...
           case when primary_person_legacy_id is not null then
             md5('soyagaci:' || p_family_slug || ':person:' || primary_person_legacy_id)::uuid
           else null end as primary_person_id
    from input
  ),
  ...
  insert into public.partnership_revisions (
    id, partnership_id, status, person1_id, person2_id, primary_person_id,
...
-- Populate primary_person_id for existing partnership revisions
alter table public.partnership_revisions disable trigger partnership_revisions_immutable;
update public.partnership_revisions
set primary_person_id = person1_id
where primary_person_id is null;
alter table public.partnership_revisions enable trigger partnership_revisions_immutable;
